...
import {
  generateKeyPair, exportJWK, SignJWT, createLocalJWKSet, jwtVerify,
} from 'jose';
import { AccessDeniedError } from '../../src/auth/AccessDeniedError.js';
import { authenticate, authorize } from '../../src/auth/authzn.js';
import { ADMIN_CLIENT_ID } from '../../src/auth/clients.js';
import { AuthInfo } from '../../src/auth/auth-info.js';
import { PERMISSIONS } from '../../src/auth/permissions.js';
import {
  getSiteAccessConfig, getSiteAuthToken, getTransientSiteTokenInfo,
  getHelix4SiteAuthToken, BEARER_IDP,
...
describe('Authentication Test', () => {
  const suffix = '/org/sites/site/status/index.md';
  const DEFAULT_CONTEXT = () => createContext(suffix, {
    attributes: { authInfo: undefined },
  });
  ...
  it('rejects too old id_token', async () => {
    ...
    const authInfo = await authenticate(DEFAULT_CONTEXT(), {
      ...DEFAULT_INFO,
      cookies: {
        auth_token: `idp_name=test&id_token=${idToken}`,
      },
    });
  ...
  it('supports the api token', async () => {
    const authInfo = await testApiToken('owner/repo', '1234', '1234');
    assert.deepStrictEqual(authInfo.profile, {
  ...
  it('supports the api token for owner', async () => {
    const authInfo = await testApiToken('owner/*', '1234', '1234');
    assert.deepStrictEqual(authInfo.profile, {
      aud: '452733d4-6ae5-4590-8d0f-27404a03aca8',
      email: 'helix@adobe.com',
      iss: 'urn:example:issuer',
  ...
  it('supports org-wide api token', async () => {
    ...
    ctx.attributes.configAll.config = {
      data: {
        admin: {
          apiKeyId: ['1234'],
        },
  ...
  it('rejects org-wide api token missing config all', async () => {
    ...
    BEARER_IDP.token = idpFakeTestIDP;
    const ctx = DEFAULT_CONTEXT();
    ctx.attributes.configAll = undefined;
    const authInfo = await authenticate(ctx, {
      ...DEFAULT_INFO,
      route: 'config',
      org: 'org',
