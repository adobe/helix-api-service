...
import {
  ...
  getIndexTargets, getFetchHeaders, getRetryParams,
} from './utils.js';
 ...
 * @property {import('fetch-retry').RequestInitRetryParams} retryParams retry params
...
export class IndexJob extends Job {
  ...
  async prepare() {
    ...
    let index = await fetchExtendedIndex(context, info);
    index = {
      indices: index.indices
        .filter(
          ({ target }) => (target?.replace(/^s3:\//, '')?.startsWith('/')),
        )
    ...
    const retryParams = await getRetryParams(context);
  ...
  async reindexRecord(bulkContext, webPath, indexRecord) {
    ...
    const { log } = context;
    const {
      headers, index, messages, retryParams,
    } = bulkContext;

    const fetchURL = info.getLiveUrl(webPath);
    const page = await fetchPage(context, fetchURL, headers, retryParams);
    if (page.error) {
