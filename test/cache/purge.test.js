...
import {
  ...
  purge, purgeCode, purgeContent,
  purgeRedirects, performPurge,
  surrogatePurge, urlPurgeHlxPage,
...
const TEST_CONTEXT = () => DEFAULT_CONTEXT({
  env: {
...
describe('Purge Tests (url)', () => {
  const DEFAULT_INFO = createPathInfo('/cache/owner/repo/ref', 'POST');
  ...
  it('purges preview and live url', async () => {
    ...
    const result = await purge(TEST_CONTEXT(), DEFAULT_INFO, PURGE_PREVIEW_AND_LIVE);
    assert.strictEqual(result.status, 200);
  });
  ...
  it('purges live url with html extension', async () => {
    ...
    const result = await purge(TEST_CONTEXT(), {
      ...DEFAULT_INFO,
      path: '/products/abc-344',
      rawPath: '/products/ABC-344.html',
      resourcePath: '/products/abc-344.md',
    }, PURGE_LIVE);
    assert.strictEqual(result.status, 200);
  });
  ...
  it('purges page url', async () => {
    nock('https://api.fastly.com')
      .post('/purge/ref--repo--owner.hlx.page/helix-config.json')
      .reply(function f() {
        assert.strictEqual(this.req.headers['fastly-key'], '1234');
        return [200];
      })
      .post('/purge/ref--repo--owner.hlx-fastly.page/helix-config.json')
      .reply(function f() {
        assert.strictEqual(this.req.headers['fastly-key'], '1234');
        return [200];
      });
  ...
  it('purges redirect paths', async () => {
    ...
    const result = await purgeRedirects(TEST_CONTEXT(), DEFAULT_INFO, [
      '/home.md',
      '/foo.html',
      '/blog/index.md',
      '/just_in_case',
