...
export async function getContentBusInfo(context, info, partition) {
  ...
  if (response.ok) {
    ...
    ret.lastModified = response.headers.get('last-modified');
    ret.sourceLocation = response.headers.get('x-source-location');
    ret.sourceLastModified = response.headers.get('x-source-last-modified') || undefined;
    ret.sheetNames = response.headers.get('x-sheet-names') || undefined;
    ret.redirectLocation = response.headers.get('redirect-location') || undefined;

    if (partition === 'preview' && info.resourcePath.startsWith('/.snapshots/')) {
      ret.lastPreviewed = response.headers.get('x-last-previewed') || undefined;
      ret.lastPublished = response.headers.get('x-last-published') || undefined;
    }
    ...
    ret.error = response.headers.get('x-error');
  }
