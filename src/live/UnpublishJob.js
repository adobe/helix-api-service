/*
 * Copyright 2026 Adobe. All rights reserved.
 * This file is licensed to you under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License. You may obtain a copy
 * of the License at https://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software distributed under
 * the License is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR REPRESENTATIONS
 * OF ANY KIND, either express or implied. See the License for the specific language
 * governing permissions and limitations under the License.
 */
import processQueue from '@adobe/helix-shared-process-queue';
import { HelixStorage } from '@adobe/helix-shared-storage';

import purge, { PURGE_LIVE } from '../cache/purge.js';
import { getMetadataPaths, REDIRECTS_JSON_PATH, PURGE_ALL_CONTENT_THRESHOLD } from '../contentbus/contentbus.js';
import { Job } from '../job/Job.js';
import { fetchExtendedIndex } from '../index/utils.js';
import indexRemove from '../index/remove.js';
import { publishBulkResourceNotification } from '../support/notifications.js';
import { RequestInfo, toResourcePath, toWebPath } from '../support/RequestInfo.js';
import { UnpublishResource } from './UnpublishResource.js';

/**
 * Concurrency to use when unpublishing resources.
 */
const JOB_CONCURRENCY = 4;

/**
 * Job that deletes a bulk of live resources in the background.
 */
export class UnpublishJob extends Job {
  /**
   * Topic of this job.
   */
  static TOPIC = 'live-remove';

  /**
   * Prepare the bulk operation by gathering all paths in the live partition.
   *
   * @param {Array<import('../support/utils.js').PrefixOrPath>} paths paths to remove
   * @param {string} contentBusId content bus ID
   * @param {HelixStorage} storage storage to use
   * @returns {Promise<Array<UnpublishResource>>} resources to delete
   */
  async prepare(paths, contentBusId, storage) {
    const { ctx } = this;

    const metadataPaths = getMetadataPaths(ctx);
    const resources = [];

    const excluded = (path) => metadataPaths.includes(path)
      || path.startsWith('/.helix/')
      || path.startsWith('/.snapshots/')
      || path === REDIRECTS_JSON_PATH;

    for (const { prefix, path: webPath } of paths) {
      if (prefix) {
        // eslint-disable-next-line no-await-in-loop
        const entries = await storage.list(`${contentBusId}/live${prefix}`);
        const mapped = entries
          .map((entry) => ({ ...entry, path: `${prefix}${entry.path}` }))
          .filter(({ path }) => !excluded(path))
          // eslint-disable-next-line max-len
          .map(({ lastModified, path }) => new UnpublishResource(path, toWebPath(path), lastModified));
        resources.push(...mapped);
      } else if (!excluded(webPath)) {
        const resourcePath = toResourcePath(webPath);
        const key = `${contentBusId}/live${resourcePath}`;

        // eslint-disable-next-line no-await-in-loop
        const { LastModified: lastModified } = await storage.head(key) ?? {};
        if (lastModified) {
          resources.push(new UnpublishResource(resourcePath, webPath, lastModified));
        }
      }
    }
    return resources;
  }

  /**
   * Process a single resource that should be deleted from live.
   *
   * @param {UnpublishResource} resource resource
   */
  async processResource(resource) {
    const { ctx, info } = this;
    const { webPath } = resource;

    const start = Date.now();
    const localInfo = RequestInfo.clone(info, { path: webPath, route: 'live' });
    const res = await resource.process(ctx, localInfo);
    const stop = Date.now();
    if (res.ok) {
      await this.audit(ctx, localInfo, { res, start, stop });
    }
  }

  /**
   * Delete all resources from the live partition.
   *
   * @returns {Promise<void>}
   */
  async delete() {
    const { state } = this;

    // eslint-disable-next-line max-len
    await processQueue([...state.data.resources], async (/** @type {UnpublishResource} */ resource) => {
      if (await this.checkStopped()) {
        return;
      }
      await this.processResource(resource);
      state.progress.processed += 1;
      await this.writeStateLazy();
    }, JOB_CONCURRENCY);
  }

  /**
   * Purge the CDN cache for all successfully unpublished resources.
   *
   * @param {string} contentBusId content bus id
   * @returns {Promise<void>}
   */
  async purgeResources(contentBusId) {
    const { ctx, info, state: { data: { resources } } } = this;

    const removedPaths = resources.filter((r) => r.isDeleted()).map(({ webPath }) => webPath);
    if (removedPaths.length > PURGE_ALL_CONTENT_THRESHOLD) {
      await purge.perform(ctx, info, [{ key: contentBusId }], PURGE_LIVE, 'main');
    } else {
      await purge.content(ctx, info, removedPaths, PURGE_LIVE);
    }
  }

  /**
   * Remove index entries and send notifications for all successfully unpublished resources.
   *
   * @returns {Promise<void>}
   */
  async index() {
    const { ctx, info, state: { data } } = this;

    const removedResources = data.resources.filter((r) => r.isDeleted());
    const idx = await fetchExtendedIndex(ctx, info);
    await processQueue([...removedResources], async (resource) => {
      const localInfo = RequestInfo.clone(info, { path: resource.webPath, route: 'live' });
      if (idx) {
        await indexRemove(ctx, localInfo, idx);
      }
    }, JOB_CONCURRENCY);

    const resourcePaths = removedResources.map(({ resourcePath }) => resourcePath);
    await publishBulkResourceNotification(ctx, 'resources-unpublished', info, resourcePaths, data.resources);
    await this.writeStateLazy();
  }

  /**
   * Runs the bulk unpublish job.
   *
   * @return {Promise<void>}
   */
  async run() {
    const { ctx } = this;
    const { contentBusId } = ctx;
    const { data, data: { paths } } = this.state;

    const storage = HelixStorage.fromContext(ctx).contentBus();

    if (data.phase === 'collecting') {
      throw new Error('job cannot be resumed during the collecting phase. please provide a smaller input set.');
    }

    // hydrate plain objects from JSON.parse back into UnpublishResource instances
    data.resources = UnpublishResource.fromJSONArray(data.resources);

    if (!data.phase) {
      await this.setPhase('collecting');
      data.resources = await this.prepare(paths, contentBusId, storage);
      await this.trackProgress({ total: data.resources.length });
      await this.setPhase('deleting');
    }

    if (data.phase === 'deleting') {
      await this.delete();
      await this.setPhase('purging');
    }

    if (await this.checkStopped()) {
      return;
    }

    if (data.phase === 'purging') {
      await this.purgeResources(contentBusId);
      await this.setPhase('indexing');
    }

    if (data.phase === 'indexing') {
      await this.index();
      await this.setPhase('completed');
    }
  }
}
