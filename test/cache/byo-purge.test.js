...
describe.skip('BYO CDN Purge Tests', () => {
  ...
  it('handles error from invalid configuration', async () => {
    // nock.projectConfig('foo-id', CONFIG_INVALID);
    // live
    ...
    const result = await purge.resource(TEST_CONTEXT(), DEFAULT_INFO, PURGE_LIVE);
    assert.strictEqual(result.status, 502);
  });

  it('ignores purge with no cdn.type', async () => {
    nock.projectConfig('foo-id', CONFIG_INVALID_NO_TYPE);
    // live
    nock('https://api.fastly.com')
      .intercept('/service/1PluOUd9jqp1prQ8PHd85n/purge', 'POST')
      .reply(200, { status: 'ok', id: '12345' })
  ...
  it('ignores purge with no configAll', async () => {
    // live
    nock('https://api.fastly.com')
      .intercept('/service/1PluOUd9jqp1prQ8PHd85n/purge', 'POST')
      .reply(200, { status: 'ok', id: '12345' })
    ...
    ctx.attributes.configAll = null;
    const result = await purge(ctx, DEFAULT_INFO, PURGE_LIVE);
    assert.strictEqual(result.status, 200);
  });

  it('ignores purge with invalid purge client config', async () => {
    nock.projectConfig('foo-id', CONFIG_CLOUDFLARE_INVALID);
    // live
    nock('https://api.fastly.com')
      .intercept('/service/1PluOUd9jqp1prQ8PHd85n/purge', 'POST')
      .reply(200, { status: 'ok', id: '12345' })
