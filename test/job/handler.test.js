...
describe('Job Handler Tests', () => {
  ...
  it('GET returns live-publish job status', async () => {
    nock.siteConfig(SITE_CONFIG);
    nock.content()
      .getObject('/preview/.helix/admin-jobs/live-publish/incoming/job-24.json')
      .reply(200, {
        state: 'running',
        data: {
          paths: ['/document'],
    ...
    const result = await main(new Request('https://api.aem.live/'), {
      ...
      pathInfo: {
        suffix: '/org/sites/site/jobs/live-publish/job-24',
      },
    });
    ...
    assert.deepStrictEqual(await result.json(), {
      links: {
        list: 'https://api.aem.live/org/sites/site/jobs/live-publish/',
        self: 'https://api.aem.live/org/sites/site/jobs/live-publish/job-24',
        details: 'https://api.aem.live/org/sites/site/jobs/live-publish/job-24/details',
      },
      state: 'running',
    });
  });
