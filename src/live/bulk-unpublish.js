...
/**
 * Handles bulk unpublish of live resources.
 *
 * @param {import('../support/AdminContext').AdminContext} context the universal context
 * @param {import('../support/RequestInfo').RequestInfo} info request info
 * @returns {Promise<Response>} response
 */
export default async function bulkUnpublish(context, info) {
  const { log } = context;

  const paths = context.data.paths ?? [];
  if (paths.length === 0) {
    return errorResponse(log, 400, "bulk-unpublish payload is missing 'paths'.");
  }
  if (!Array.isArray(paths)) {
    return errorResponse(log, 400, "bulk-unpublish 'paths' is not an array.");
  }
  ...
  return Job.create(context, info, UnpublishJob.TOPIC, {
    transient: true,
    jobClass: UnpublishJob,
    data: {
