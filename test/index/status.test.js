...
describe('Index Status Tests', () => {
  ...
  beforeEach(() => {
    ...
    nock.content().head('/live/sitemap.json').reply(404);
  });
  ...
  it('reports error if index definition is not found', async () => {
    ...
    assert.deepStrictEqual(response.headers.plain(), {
      ...
      'x-error': 'no index configuration could be loaded for document org/site/document.md',
    });
  });
  ...
  it('rejects indexing an index target', async () => {
    nock.indexConfig(INDEX_CONFIG);
    ...
    assert.deepStrictEqual(response.headers.plain(), {
      'cache-control': 'no-store, private, must-revalidate',
      'content-type': 'text/plain; charset=utf-8',
      'x-error': 'index targets can not be indexed',
    });
  });
});
